...
def remove_person_from_image(image_path):
    image = cv2.imread(image_path)
    if image is None:
        raise ValueError("Invalid image path or image cannot be loaded.")
    ...
    save_path = os.path.join(SAVE_DIR, "test_human_remove.png")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python removehuman.py <image_path>")
        sys.exit(1)
    image_path = sys.argv[1]
    remove_person_from_image(image_path)
