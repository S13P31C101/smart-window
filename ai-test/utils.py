...
async def notify_music_callback(media_id: int, device_id: str, music_url: str):
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        resp = await client.post(AI_CALLBACK_URL, json=payload, headers=headers)
        if resp.status_code not in (200, 201):
            raise Exception(f"Music callback failed: {resp.status_code}, {resp.text}")

async def handle_recommend_music(request):
    image_bytes = request.get("image_bytes")
    media_id = request.get("mediaId")
    device_id = request.get("targetAIS3Key")
    if not image_bytes or not media_id or not device_id:
        return {"success": False, "error": "image_bytes, mediaId, targetAIS3Key(deviceId) are required"}
    try:
        caption = extract_mood_caption(image_bytes)
        result = await search_youtube_music(caption + " piano music")
        music_url = result["url"] if result else None
        if music_url:
            try:
                await notify_music_callback(media_id, str(device_id), music_url)
            except Exception as e:
                tb = traceback.format_exc()
                return {
                    "success": False,
                    "musicUrl": music_url,
                    "message": "music callback failed",
                    "error": str(e)
                }
            return {
                "success": True,
